...
class CookieManager {
    public static function setLoggedInCookie($username) {
        setcookie('loggedin', 'true', time() + 3600, '/'); // Crée un cookie "loggedin" valide pendant 1 heure
        setcookie('username', $username, time() + 3600, '/'); // Crée un cookie "username" valide pendant 1 heure
